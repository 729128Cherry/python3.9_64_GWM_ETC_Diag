...
def can_net_start(zcanlib, device_handle, chn):
    ip = zcanlib.GetIProperty(device_handle)
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())," : 获取配置指针：%s" % (ip))
    ret = zcanlib.SetValue(ip, str(chn) + "/work_mode", '0')
    if ret != ZCAN_STATUS_OK:
        print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())," :Set CH%d CANFD work_mode! failed" % (chn))
    ret = zcanlib.SetValue(ip, str(chn) + "/ip", '192.168.0.178')
    if ret != ZCAN_STATUS_OK:
        print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())," :Set CH%d CANFD ip! failed" % (chn))
    ret = zcanlib.SetValue(ip, str(chn) + "/work_port", "8000")
    if ret != ZCAN_STATUS_OK:
        print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())," :Set CH%d CANFD work_port failed!" % (chn))
    zcanlib.ReleaseIProperty(ip)
    chn_init_cfg = ZCAN_CHANNEL_INIT_CONFIG()
    chn_init_cfg.can_type = ZCAN_TYPE_CANFD
    #chn_init_cfg.config.canfd.abit_timing = 0x0001975e #1Mbps
