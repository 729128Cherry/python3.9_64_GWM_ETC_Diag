...
class ZLG:
	...
	def write_vin(self):
		# 扩展会话
		self.more_frams(dev_fd["chn_handle"], "1003")
		ret = self.deal_flash("1003", 4, "5003")
		if ret[0] == 255:
			return 255
		print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), f" ： 进入扩展会话！")

		# 安全访问
		self.transform_device_event_sec(dev_fd["chn_handle"])
		ret = self.deal_flash("2701", 4, "6702")
		if ret[0] == 255:
			return 255
		print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), f" ： 进入安全模式！")

		# 写入VIN
		self.more_frams(dev_fd["chn_handle"], "2ef190"+VIN)
		ret = self.deal_flash("2ef190", 6, "6ef190")
		if ret[0] == 255:
			return 255
		print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), f" ： VIN码写入成功！")

	@With_Thread
	def read_vin(self):
		self.more_frams(dev_fd["chn_handle"], "22f190")
		ret = self.deal_flash("22f190", 6, "62f190")
		if ret[0] == 255:
			return 255
		print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), f" ： VIN码读取成功！")
...
class GUI(tk.Tk, ZLG):
	...
	def _Third_datainput_WidgetsInit(self):
		...
		self.device_open_button.grid(row=0, column=0, padx=5, pady=2, sticky=tk.E + tk.W)
		...
		self.device_close_button.grid(row=0, column=1, padx=4, pady=2, sticky=tk.E + tk.W)
		...
		self.diag_secuity_button.grid(row=4, column=0, padx=2, pady=1, sticky=tk.E + tk.W)
		...
		self.diag_con_button.grid(row=4, column=1, padx=2, pady=1, sticky=tk.E + tk.W)

		self.device_display_lable = tk.Label(self._Second_datainput, text="设备:", justify="left", anchor="w")
		self.device_display_lable.grid(row=5, column=0, columnspan=2, padx=1.5, pady=1, sticky=tk.EW)

		self.len_display_lable = tk.Label(self._Second_datainput, text="长度:", justify="left", anchor="w")
		self.len_display_lable.grid(row=6, column=0, columnspan=2, padx=1.5, pady=1, sticky=tk.EW)

		self.diag_secuity_button = tk.Button(self._Second_datainput, width=4, text='写入VIN',
												command=self.write_vin)
		self.diag_secuity_button.grid(row=7, column=0, padx=2, pady=1, sticky=tk.E + tk.W)

		self.diag_con_button = tk.Button(self._Second_datainput, width=4, text='读取VIN',
												command=self.read_vin)
		self.diag_con_button.grid(row=7, column=1, padx=2, pady=1, sticky=tk.E + tk.W)
